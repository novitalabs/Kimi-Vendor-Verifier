...
def model():
    if not _SMODEL:
        raise pytest.UsageError("SMODEL env var is required")
    return _SMODEL
...
def _consume_stream(
    ...
    *,
    ...
    require_usage_details: bool = True,
) -> dict:
    ...
    for frame in _iter_sse_frames(response):
        ...
        if not choices:
            ...
            usage = payload["usage"]
            # Summary frame normally requires the full detail objects (P0.1 + P0.2);
            # callers may relax this for models that don't return the details (e.g. kimi-k3).
            _assert_usage_shape(
                usage,
                "summary usage",
                require_details=require_usage_details,
            )
    ...
    **Spec:** P0.1 (include_usage 汇总帧) · P0.2 (usage 字段类型)"""
    ...
    with _request_stream(
        ...
        stats = _consume_stream(
            ...
            require_usage_details=(model != "kimi-k3"),
        )
